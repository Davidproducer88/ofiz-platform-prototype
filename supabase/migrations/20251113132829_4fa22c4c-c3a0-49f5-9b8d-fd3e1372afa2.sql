...
CREATE INDEX IF NOT EXISTS idx_founder_codes_user ON public.founder_discount_codes(user_id);
CREATE INDEX IF NOT EXISTS idx_founder_codes_code ON public.founder_discount_codes(code);
CREATE INDEX IF NOT EXISTS idx_founder_codes_active ON public.founder_discount_codes(is_active);
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'founder_discount_codes' 
    AND policyname = 'Users can view own discount codes'
  ) THEN
...
-- Índices
CREATE INDEX IF NOT EXISTS idx_code_usage_code ON public.founder_code_usage(code_id);
CREATE INDEX IF NOT EXISTS idx_code_usage_user ON public.founder_code_usage(used_by_user_id);
CREATE INDEX IF NOT EXISTS idx_code_usage_booking ON public.founder_code_usage(booking_id);
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'founder_code_usage' 
    AND policyname = 'Users can view own code usage'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'founder_code_usage' 
    AND policyname = 'System can insert code usage'
  ) THEN
...
BEGIN
  ...
  LOOP
    new_code := COALESCE(profile_name, 'FOUN') || UPPER(SUBSTRING(MD5(RANDOM()::TEXT), 1, 6));
    SELECT EXISTS(
...
BEGIN
  ...
  IF code_record.max_uses IS NOT NULL AND code_record.times_used >= code_record.max_uses THEN
    RETURN jsonb_build_object('valid', false, 'error', 'Código ha alcanzado el límite de usos');
  END IF;
...
AS $$
BEGIN
  INSERT INTO public.founder_code_usage (code_id, used_by_user_id, booking_id, discount_amount)
  VALUES (code_id_param, used_by_user_id_param, booking_id_param, discount_amount_param);
